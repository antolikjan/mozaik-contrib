...
def verify_push_pull(data_store, connection_name):
    # test the push pull
    conn = data_store.get_analysis_result(
        identifier='Connections', proj_name=connection_name)
    assert len(conn) == 1
    conn = conn[0]
    OR_source = data_store.get_analysis_result(
        sheet_name=conn.source_name, identifier='PerNeuronValue', value_name='LGNAfferentOrientation')
    PHASE_source = data_store.get_analysis_result(
        sheet_name=conn.source_name, identifier='PerNeuronValue', value_name='LGNAfferentPhase')
    OR_target = data_store.get_analysis_result(
        sheet_name=conn.target_name, identifier='PerNeuronValue', value_name='LGNAfferentOrientation')
    PHASE_target = data_store.get_analysis_result(
        sheet_name=conn.target_name, identifier='PerNeuronValue', value_name='LGNAfferentPhase')
    ...
    pylab.figure()
    angles, mag = circ_mean(numpy.tile(
        OR_source, (conn.weights.shape[1], 1)).T, conn.weights, high=numpy.pi, axis=0)
    pylab.subplot(1, 2, 1)
    ...
    angles, mag = circ_mean(numpy.tile(
        PHASE_source, (conn.weights.shape[1], 1)).T, conn.weights, high=numpy.pi*2, axis=0)
    pylab.subplot(1, 2, 2)
...
def verify_connectivity(data_store):
    ...
    ConnectivityPlot(data_store, ParameterSet({'neuron': data_store.get_sheet_ids('V1_Exc_L4', find_neuron(
        'center', l4_pos)), 'sheet_name': 'V1_Exc_L4', 'reversed': True}), param_filter_query(data_store, identifier='PerNeuronValue', value_name='LGNAfferentOrientation')).plot()
    ConnectivityPlot(data_store, ParameterSet({'neuron': data_store.get_sheet_ids('V1_Exc_L4', find_neuron(
        'top_right', l4_pos)), 'sheet_name': 'V1_Exc_L4', 'reversed': True}), param_filter_query(data_store, identifier='PerNeuronValue', value_name='LGNAfferentOrientation')).plot()
    ConnectivityPlot(data_store, ParameterSet({'neuron': data_store.get_sheet_ids('V1_Exc_L4', find_neuron(
        'top_left', l4_pos)), 'sheet_name': 'V1_Exc_L4', 'reversed': True}), param_filter_query(data_store, identifier='PerNeuronValue', value_name='LGNAfferentOrientation')).plot()
    ConnectivityPlot(data_store, ParameterSet({'neuron': data_store.get_sheet_ids('V1_Exc_L4', find_neuron(
        'bottom_right', l4_pos)), 'sheet_name': 'V1_Exc_L4', 'reversed': True}), param_filter_query(data_store, identifier='PerNeuronValue', value_name='LGNAfferentOrientation')).plot()
    ConnectivityPlot(data_store, ParameterSet({'neuron': data_store.get_sheet_ids('V1_Exc_L4', find_neuron(
        'bottom_left', l4_pos)), 'sheet_name': 'V1_Exc_L4', 'reversed': True}), param_filter_query(data_store, identifier='PerNeuronValue', value_name='LGNAfferentOrientation')).plot()
