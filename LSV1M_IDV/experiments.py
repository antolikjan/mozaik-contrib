...
def create_experiments(model):
    ...
    return  [
                ...
                MeasureOrientationTuningFullfield(model,ParameterSet({'num_orientations' : 10,'spatial_frequency' : 0.8,'temporal_frequency':2,'grating_duration':2*143*7,'contrasts':[30,100],'num_trials':10})),
...
def create_experiments_or(model):
    ...
    return  [
                           ...
                           # Measure orientation tuning with full-filed sinusoidal gratins
                           MeasureOrientationTuningFullfield(model,ParameterSet({'num_orientations':2,'spatial_frequency':0.8,'temporal_frequency':2,'grating_duration':2*143*7,'contrasts':[100,10],'num_trials': 5})),
...
def create_experiments_stc(model):
    ...
    return  [
                           ...
                           #Size Tuning  
                           MeasureSizeTuning(model,ParameterSet({'num_sizes': 12,'max_size': 3.0,'log_spacing' : True,'orientation' : 0,'spatial_frequency' : 0.8,'temporal_frequency' : 2,'grating_duration' : 2*143*7,'contrasts' : [30,100],'num_trials' : 10,'with_flat': False})),
            ]
...
def create_experiments_octc(model):
    ...
    return  [
                           ...
                           #OCTC
                           MeasureOrientationContrastTuning(model,ParameterSet({'num_orientations' : 8,'orientation' : 0,'center_radius' : 0.9,'surround_radius' :20.0,'spatial_frequency' : 0.8,'temporal_frequency' : 2,'grating_duration' : 10*2*143*7,'contrasts' : [100],'num_trials' : 1})),
            ]
